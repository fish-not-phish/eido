...
This guide explains how to deploy the Eido stack (Next.js frontend, Django backend, Postgres database) behind a **Traefik reverse proxy** with automatic HTTPS certificates from Let’s Encrypt via Cloudflare DNS challenge.
...
services:
  backend:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.eido-backend.rule=Host(`eido.my-domain.com`) && PathPrefix(`/backend`)"
